...
public class ListaEsperaSocioModel {
	...
	/*
	 * Comprueba si un socio ya esta en la lista de espera de una actividad
	 */
	public boolean estaEnListaEspera(int idSocio, int idActividad) {
		String sql = "SELECT COUNT(*) FROM ListaEspera WHERE id_socio = ? AND id_actividad = ?";
		List<Object[]> res = db.executeQueryArray(sql, idSocio, idActividad);
		return ((Number) res.get(0)[0]).intValue() > 0;
	}

	/*
	 * Añade a un socio a la lista de espera
	 */
	public void apuntarListaEspera(int idSocio, ActividadInscripcionDTO actividad) {
		// Validaciones previas
		if (!estaAlCorriente(idSocio)) {
			throw new ApplicationException("No puedes apuntarte: No estás al corriente de pago.");
		}
		if (estaInscrito(idSocio, actividad.getIdActividad())) {
			throw new ApplicationException("Ya estás inscrito en esta actividad.");
		}
		if (estaEnListaEspera(idSocio, actividad.getIdActividad())) {
			throw new ApplicationException("Ya te encuentras en la lista de espera para esta actividad.");
		}
		
		// Insertar en la lista de espera (fecha_solicitud se genera sola con CURRENT_TIMESTAMP)
